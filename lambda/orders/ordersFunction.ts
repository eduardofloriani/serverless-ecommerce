...
export async function handler(event: APIGatewayProxyEvent, context: Context): Promise<APIGatewayProxyResult> {
    ...
    if (method === 'GET') {
        if (event.queryStringParameters) {
            ...
            if (email) {
                if (orderId) {
                    ...
                    return {
                        statusCode: 200,
                        body: JSON.stringify(orders.map(convertOrderToOrderResponse)),
                    };
                }
            }
        } else {
        ...
        try {
            ...
            return {
                statusCode: 200,
                body: JSON.stringify(convertOrderToOrderResponse(orderDeleted)),
            };
        } catch (error) {
            console.log((<Error>error).message);
            return {
...
function convertOrderToOrderResponse(order: Order): OrderResponse {
    ...
    const orderResponse: OrderResponse = {
        ...
        products: order.products?.length ? orderProducts : undefined,
        billing: {
